...
const readFileAndPost = (file, options) => new Promise((resolve, reject) => {
    ...
    return {
        ...
        upload: {
            catalog: (modelId, name, file) => readFileAndPost(file, {
                ...
                if (results.errorLineCount > 0 || (results.errorSummary && results.errorSummary.length > 0)) {
                    console.warn(`Catalog imported with ${results.errorLineCount} errors.\n${(results.errorSummary || []).join('\n')} `);
                } else {
                    console.info(`Succesfully imported ${results.importedLineCount} catalog entries`);
                }
            }),
            ...
            usage: (modelId, name, file) => readFileAndPost(file, {
                ...
                if (results.errorLineCount > 0 || (results.errorSummary && results.errorSummary.length > 0)) {
                    console.warn(`Usage data imported with ${results.errorLineCount} errors.\n${(results.errorSummary || []).join('\n')} `);
                } else {
                    console.info(`Succesfully imported ${results.importedLineCount} sales records`);
                }
            })
        },
